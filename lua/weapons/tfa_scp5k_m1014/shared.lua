...
SWEP.VMPos                  = Vector(-0.5, 0, 0.75)+Vector(1,-2,-2)    --The viewmodel positional offset, constantly.  Subtract this from any other modifications to viewmodel position.
...
SWEP.ViewModelPunch_MaxVertialOffset             = 0.15 -- Default value is 3
...
SWEP.Primary.SpreadRecovery                      = 1.25 --How much the spread recovers, per second. Example val: 3
...
local SightsOffsetThingy = Vector(0.205,0,0.75)
local SightsOffsetThingyButForAngles = Vector(-0.275,0.05,0)
...
local modrail_s_pos = Vector(0, -1, 0.8)
local modrail_ua_pos = Vector(0, -17.5, -1.8)
local modrail_ug_pos = Vector(0, 1.5, -0.1)
local modrail_la_pos = Vector(-0.5, -17.5, -1.2)
local modrail_lg_pos = Vector(-0.1, 1.5, 0)
local modrail_ra_pos = Vector(0.5, -17.5, -1.2)
...
SWEP.VElements = {
	...
	["suppressor"] = { type = "Model", model = "models/weapons/tfa_ins2/upgrades/a_suppressor_sec.mdl", bone = "barrel", rel = "", pos = Vector(0, -8, -0.25), angle = Angle(0, 90, 0), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {}, active = false, bonemerge = false },
