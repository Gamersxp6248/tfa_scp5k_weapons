...
SWEP.VMPos                  = Vector(-2.5, -1, 0.5)+Vector(2,-2,-2)    --The viewmodel positional offset, constantly.  Subtract this from any other modifications to viewmodel position.
...
-------------------------- Recoil
SWEP.Primary.KickUp                              = 0.03 -- This is the maximum upwards recoil (rise)
...
local SightsOffsetThingy = Vector(0.095,-6,0.71)
local SightsOffsetThingyButForAngles = Vector(-0.125,-0.12,0)
...
local modrail_s_pos = Vector(0, -0.5, 2.2)
local modrail_ua_pos = Vector(0, -1.5, 0.35)
local modrail_ug_pos = Vector(0, -2, -0.1)
local modrail_la_pos = Vector(-0.5, -1.5, 0.65)
local modrail_lg_pos = Vector(-0.1, -2, 0)
local modrail_ra_pos = Vector(0.5, -1.5, 0.65)
...
SWEP.VElements = {
	...
	["suppressor"] = { type = "Model", model = "models/weapons/tfa_ins2/upgrades/a_suppressor_sec.mdl", bone = "barrel", rel = "", pos = Vector(0, -4.5, 0), angle = Angle(0, 90, 0), size = Vector(0.5,0.5,0.5), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {}, active = false, bonemerge = false },
