...
--------------------------------------------------- Viewmodel
SWEP.ViewModel			    = "models/weapons/tfa_scp5k/mk17/v_mk17.mdl" 
SWEP.ViewModelFOV			= 80		         -- This controls how big the viewmodel looks.  Less is more.
SWEP.ViewModelFlip			= false		         -- Set this to true for CSS models, or false for everything else (with a righthanded viewmodel.)
SWEP.UseHands               = true               --Use gmod c_arms system.
SWEP.VMPos                  = Vector(-1.25, 0.5, 0.6)+Vector(1,-2,-2)    --The viewmodel positional offset, constantly.  Subtract this from any other modifications to viewmodel position.
...
SWEP.PrintName			= "MK 17"		         -- Weapon name (Shown on HUD)
...
SWEP.Primary.RangeFalloff          = 0.8           -- Set to 0.8, for example, to start falling off after 80% of the range.
...
SWEP.Primary.StaticRecoilFactor    = 0.3       --Amount of recoil to directly apply to EyeAngles.  Enter what fraction or percentage (in decimal form) you want.
...
SWEP.Primary.SpreadIncrement       = 0.28 --What percentage of the modifier is added on, per shot.  Example val: 1/3.5
...
--------------------------------------------------- Sounds
SWEP.Primary.Sound             = Sound("Weapon_MK17_Fire")
...
SWEP.EventTable = {
	...
	[ACT_VM_RELOAD] = {
		{time = 0 / 60, type = "sound", value = Sound("Weapon_MK17_MagPouchDown")},
		{time = 45 / 60, type = "sound", value = Sound("Weapon_MK17_MagOut")},
		--{time = 45 / 60, type = "sound", value = Sound("Weapon_MK18_MagHit")},
		{time = 80 / 60, type = "sound", value = Sound("Weapon_MK17_MagIn")},
		{time = 110 / 60, type = "sound", value = Sound("Weapon_MK17_MagPouchUp")},
		{time = 125 / 60, type = "sound", value = Sound("Weapon_MK17_HandlingBarrel")}
	},
	[ACT_VM_RELOAD_EMPTY] = {
		{time = 0 / 60, type = "sound", value = Sound("Weapon_MK17_EmptyMagOut")},
		{time = 15 / 60, type = "sound", value = Sound("Weapon_MK17_EmptyMagPouch")},
		{time = 80 / 60, type = "sound", value = Sound("Weapon_MK17_EmptyMagIn")},
		{time = 140 / 60, type = "sound", value = Sound("Weapon_MK17_EmptyBolt")},
		{time = 170 / 60, type = "sound", value = Sound("Weapon_MK17_EmptyMagPouch")},
	},
	[ACT_VM_FIDGET] = {
		{time = 0 / 60, type = "sound", value = Sound("Weapon_MK17_Rotate")},
		{time = 100 / 60, type = "sound", value = Sound("Weapon_MK17_BoltBack")},
		{time = 170 / 60, type = "sound", value = Sound("Weapon_MK17_BoltForward")},
		
	},
	[ACT_VM_MISSCENTER] = {
		{time = 0 / 60, type = "sound", value = Sound("Weapon_MK17_MagCheckOut")},
		
	},
	[ACT_VM_MISSCENTER2] = {
		{time = 35 / 60, type = "sound", value = Sound("Weapon_MK17_MagCheckIn")},
...
local SightsOffsetThingy = Vector(-0.8315,0,0.9575)
local SightsOffsetThingyButForAngles = Vector(-0.04,-0.04,0)
...
local modrail_s_pos = Vector(0, -1, 2.8)
local modrail_ua_pos = Vector(0, -10.25, 0.25)
local modrail_ug_pos = Vector(0, 1.5, -0.1)
local modrail_la_pos = Vector(-0.9, -10.25, 1.1275)
local modrail_lg_pos = Vector(-0.1, 1.5, 0)
local modrail_ra_pos = Vector(0.9, -10.25, 1.1275)
